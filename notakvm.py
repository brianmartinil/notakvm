...
def is_prev_instance(proc):
    ...
    if os.path.realpath(sys.executable) == os.path.realpath(cmd_line[0]) and os.path.realpath(sys.argv[0]) == os.path.realpath(cmd_line[1]):
       return True

# If we are started with the parameter "launch", we:
#   * Check to see if the previous instance still seems to be around. If so, kill it.
#   * Run another version of ourselves but omit the "launch" parameter, and use
#     popen to spawn it in the background.  This will be our long-running background
#     process
if len(sys.argv) > 1 and sys.argv[1] == 'launch':
    kill_previous_instance()

    # launch ourselves without the "launch" parameter
    p = Popen([sys.executable, sys.argv[0]], stdin=PIPE, stdout=PIPE, stderr=PIPE,
